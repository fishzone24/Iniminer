...
function download_and_run_miner() {
    ...
    # 使用 pm2 启动矿机
    pm2 start $TARGET_FILE --name $MINER_NAME -- --pool "stratum+tcp://$WALLET_ADDRESS.$WORKER_NAME@pool-c.yatespool.com:31189" &> $LOG_FILE
    else
    ...
    pm2 start $TARGET_FILE --name $MINER_NAME -- --pool "stratum+tcp://$WALLET_ADDRESS.$WORKER_NAME@pool-c.yatespool.com:31189" $CPU_DEVICES &> $LOG_FILE
    fi
